...
DATA_DIR = "C:\\Users\\luluk\\OneDrive - imt-atlantique\\Imt_courses\\PROCOM\\ia_racing_imt-main\\supervise\\dataset_drive12\\images"
...
LABEL_DIR = "C:\\Users\\luluk\\OneDrive - imt-atlantique\\Imt_courses\\PROCOM\\ia_racing_imt-main\\supervise\\dataset_drive12"
...
images = []
images = [cv2.cvtColor(cv2.imread(file),cv2.COLOR_BGR2RGB) for file in natsorted(glob.glob(DATA_DIR+"\\*.jpg"))][:1000]
...
for i in range(len(images)):
    ...
    try:
        ...
        ###############################################################
        if i==0 or histogram[max(leftx_base-margin,0):min(leftx_base+margin,160)+1]==0 or histogram[max(rightx_base-margin,0):min(rightx_base+margin,160)+1]==0 or histogram.nonzero()[0][-1]-histogram.nonzero()[0][0]>85 or l>=5 or r>=5 or leftx_base==0 or rightx_base==160:          
    ...
    if i==0 or ((left_lanex_inds.shape==(0,) and left_laney_inds.shape == (0,)) and (right_lanex_inds.shape == (0,) and right_laney_inds.shape == (0,))):
        ...
        try:
            ...
            left_laney_inds = np.array([])
            right_laney_inds = np.array([])
            left_lanex_inds = np.array([])
            right_lanex_inds = np.array([])
    else:
        try:
            ...
            leftx_base = math.floor(np.mean(left_lanex_inds[:5]))
        except:
        ...
        try:     
            ...
            rightx_base = math.ceil(np.mean(right_lanex_inds))
    ...
    ##
    veh_pos_bis_img=imagex
    veh_pos_bis_img=255-cv2.inRange(veh_pos_bis_img, TRESH3_MIN, TRESH3_MAX)
    histogramveh_pos = np.sum(veh_pos_bis_img[100:120,:], axis=0)
    try:
        midpointveh = math.floor((histogramveh_pos.nonzero()[0][0]+histogram.nonzero()[0][-1])//2)
    except:
        pass
    dist=(80-midpointveh)*xm_per_pix
    ...
    if rad!=np.inf:
        ...
        radmean=np.mean(radstock)
        k+=1
    cv2.putText(resized,'Curve Radius [m]: '+ str(np.round(radmean,2)),(10,15), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,0),1,cv2.LINE_AA)
    cv2.putText(resized,'Center Offset [m]: '+ str(np.round(veh_pos,2)),(10,30), cv2.FONT_HERSHEY_PLAIN, 1,(255,0,0),1,cv2.LINE_AA)
    #cv2.putText(resized,'Lfit: '+str(np.round(left_fit_cr[0],2)),(10,45), cv2.FONT_HERSHEY_PLAIN, 1,(255,0,0),1,cv2.LINE_AA)
    #cv2.putText(resized,'Rfit: '+str(np.round(right_fit_cr[0],2)),(10,60), cv2.FONT_HERSHEY_PLAIN, 1,(255,0,0),1,cv2.LINE_AA)
    cv2.putText(resized, 'Throttle: '+str(np.round(labels[nametosave]["user/throttle"],2)),(10,75), cv2.FONT_HERSHEY_PLAIN, 1,(255,0,0),1,cv2.LINE_AA)
    ...
    start_time = time.time()
    if abs(radmean)<1.2:
        Curves.append(np.round(radmean,2))
    else:
        Curves.append(np.sign(radmean)*1.2)
    Offset.append(np.round(veh_pos,2))
    Steer.append(np.round(labels[nametosave]["user/angle"],2))
    Pos.append(dist)
    #video.write(resized)
...
plt.plot(Steer)
plt.plot(Pos)
plt.legend(("Curves","Offset","Steer", "pos"))
plt.savefig("C:\\Users\\luluk\\OneDrive\\Images\\Fig.png")
plt.show(block=True)
# video.release()
