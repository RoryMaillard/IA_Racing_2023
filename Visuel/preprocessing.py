...
DATA_DIR = "C:\\Users\\luluk\\OneDrive - imt-atlantique\\Imt_courses\\PROCOM\\ia_racing_imt-main\\supervise\\dataset_drive6\\images"
...
images = []
images = [cv2.imread(file) for file in natsorted(glob.glob(DATA_DIR+"\\*.jpg"))]
...
plt.imshow(images[0],cmap='gray', animated=True)
...
TRESH3_MAX = np.array([180, 125, 255],np.uint8)
#######################################################
...
ym_per_pix = 0.2/120 # meters per pixel in y dimension
...
for i in range(len(images)):
    ...
    try:
        ...
        ###############################################################
        if i==0 or (histogram[max(leftx_base-margin,0):min(leftx_base+margin,160)+1]==0 or histogram[max(rightx_base-margin,0):min(rightx_base+margin,160)+1]==0):
    ...
    if left_x_pos != np.inf and right_x_pos != np.inf:
        ...
        center_lanes_x_pos = left_x_pos+43

    elif right_x_pos != np.inf:
        center_lanes_x_pos = right_x_pos-43
    ...
    if ltrue:
        pts_left_lane = np.array([(np.vstack([yls+2.5, xls])).T])
        pts_left = np.array([(np.vstack([yls, xls])).T])
    else:
        pts_left = []
        pts_left_lane = []

    if rtrue:
        pts_right_lane = np.array([np.flipud((np.vstack([yrs-2.5, xrs])).T)])
        pts_right = np.array([np.flipud((np.vstack([yrs, xrs])).T)])
    else:
        pts_right = []
        pts_right_lane = []
    ...
    try:
        ...
        cv2.fillPoly(color_warp, np.int_([pts_lane]), (0, 255, 0))
    except:
        pass
    if ltrue:
        cv2.polylines(line, np.int_([pts_left]), isClosed=False, color=(255, 0, 0), thickness=5)
        cv2.polylines(linebis, np.int_([pts_left]), isClosed=False, color=(255, 255, 255), thickness=5)
    if rtrue:
        cv2.polylines(line, np.int_([pts_right]), isClosed=False, color=(255, 0, 0), thickness=5)
        cv2.polylines(linebis, np.int_([pts_right]), isClosed=False, color=(255, 255, 255), thickness=5)
    ...
    if abs(left_curverad)>=5 and abs(right_curverad)<5:
        rad=np.sign(right_curverad)*0.25+right_curverad
    elif abs(right_curverad)>=5 and abs(left_curverad)<5:
        rad=np.sign(left_curverad)*0.25+left_curverad
    elif abs(left_curverad)>=5 and abs(right_curverad)>=5:
        rad=np.inf
    cv2.putText(resized,'Curve Radius [m]: '+str(round(rad,2)),(10,15), cv2.FONT_HERSHEY_PLAIN, 1, (255,255,255),1,cv2.LINE_AA)
    cv2.putText(resized,'Center Offset [m]: '+str(round(veh_pos,2)),(10,30), cv2.FONT_HERSHEY_PLAIN, 1,(255,255,255),1,cv2.LINE_AA)
    plt.imshow(resized,animated=True)
    plt.draw()
    plt.pause(0.05-time.time()+start_time)
    plt.clf()
    plt.title('Fps:'+str(round(1/(time.time()-start_time),1)),loc='left')
    start_time = time.time()
    #video.write(resized)
